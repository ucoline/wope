...
class CustomMenu
{
    ...
    private function config($args)
    {
        ...
        if ($tpl_path && is_dir(theme_path($tpl_path))) {
            $this->path_name = trim($tpl_path, '/');
            $this->path = theme_path($tpl_path) . DS;
        }
    }
    ...
    private function outer_view($data = array(), $view = false)
    {
        ...
        if ($this->path && is_file(theme_path($filename_theme))) {
            return theme_view($this->path_name . '/outer', $data, $view);
        } else {
    ...
    private function item_view($level, $data = array())
    {
        ...
        if ($this->path) {
            ...
            if (is_file(theme_path($theme_file2))) {
                return theme_view($this->path_name . '/item-' . $level, $data, true);
            } else if (is_file(theme_path($theme_file))) {
                return theme_view($this->path_name . '/item', $data, true);
            }
        }
    ...
    private function inner_view($level, $data = array())
    {
        ...
        if ($this->path) {
            ...
            if (is_file(theme_path($theme_file2))) {
                return theme_view($this->path_name . '/inner-' . $level, $data, true);
            } else if (is_file(theme_path($theme_file))) {
                return theme_view($this->path_name . '/inner', $data, true);
            }
        }
